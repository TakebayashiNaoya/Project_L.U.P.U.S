...
# 拡張子にマッチする正規表現
# \b の代わりに (?![A-Za-z0-9]) で境界を保証し、除去漏れを防ぐ
_EXTENSIONS = re.compile(
    r'\.(' + '|'.join(re.escape(k) for k in _EXTENSION_MAP) + r')(?![A-Za-z0-9])',
    re.IGNORECASE,
)

# アルファベット・数字・アンダースコア・ハイフンからなる識別子全体にマッチする正規表現
_IDENTIFIER = re.compile(r'[A-Za-z0-9][A-Za-z0-9_\-]*')


def _replace_extension(match: re.Match) -> str:
    """拡張子マッチを _EXTENSION_MAP に従ったカタカナ読みに置換する。

    Args:
        match: _EXTENSIONS にマッチした正規表現オブジェクト。

    Returns:
        対応するカタカナ読み文字列。テーブルに存在しない場合は拡張子をそのまま返す。
    """
    return _EXTENSION_MAP.get(match.group(1).lower(), match.group(1))


def _split_identifier(word: str) -> list[str]:
    """英語識別子を命名規則に依らず単語リストに分解する。

    対応する命名規則:
        PascalCase      : SystemContext  -> ['System', 'Context']
        lowerCamelCase  : myVariable     -> ['my', 'Variable']
        snake_case      : my_variable    -> ['my', 'variable']
        SCREAMING_SNAKE : MY_VARIABLE    -> ['MY', 'VARIABLE']
        kebab-case      : my-variable    -> ['my', 'variable']
        連続大文字略語   : TTSModel       -> ['TTS', 'Model']
        数字混在        : win32api       -> ['win', '32', 'api']
        m_ プレフィックス: m_isAtHome    -> ['m', 'is', 'At', 'Home']

    Args:
        word: 分解対象の識別子文字列。

    Returns:
        分解後のトークンリスト。空トークンは除外する。
    """
    # 1. アンダースコア・ハイフン区切りをスペースに統一する
    word = re.sub(r'[_\-]+', ' ', word)
    # 2. 連続大文字略語と後続単語の境界を分割する (TTSModel -> TTS Model)
    word = re.sub(r'([A-Z]{2,})([A-Z][a-z])', r'\1 \2', word)
    # 3. 小文字/数字 → 大文字の境界を分割する (camelCase -> camel Case)
    word = re.sub(r'([a-z\d])([A-Z])', r'\1 \2', word)
    # 4. アルファベット → 数字の境界を分割する (win32 -> win 32)
    word = re.sub(r'([A-Za-z])(\d)', r'\1 \2', word)
    # 5. 数字 → アルファベットの境界を分割する (32api -> 32 api)
    word = re.sub(r'(\d)([A-Za-z])', r'\1 \2', word)
    return [t for t in word.split(' ') if t]


def _token_to_kana(token: str) -> str:
    """1トークンをカタカナに変換する。

    数字トークンはそのまま返す。
    alkana の辞書に存在しない単語や変換に失敗した場合は元のトークンを返し、
    pyopenjtalk による読みに委ねる。

    Args:
        token: 変換対象のトークン文字列。

    Returns:
        カタカナ文字列。変換できない場合は元のトークン文字列。
    """
    if token.isdigit():
        return token
    try:
        kana = alkana.get_kana(token.lower())
        return kana if kana else token
    except Exception:
        return token


def normalize_english(text: str) -> str:
    """テキスト内の英語識別子をカタカナに変換する。

    まず拡張子を _EXTENSION_MAP のカタカナ読みに置換（ドット省略）したうえで、
    PascalCase・lowerCamelCase・snake_case・SCREAMING_SNAKE・kebab-case・
    連続大文字略語・数字混在・m_ プレフィックスなど各種命名規則に対応した
    分解を行い、各トークンを alkana でカタカナ変換する。
    alkana の辞書に存在しないトークンは pyopenjtalk の読みに委ねる。

    Args:
        text: 変換対象のテキスト文字列。

    Returns:
        英語識別子をカタカナに置換したテキスト文字列。
    """
    # 拡張子をカタカナ読みに置換する（ドットは省略）
    text = _EXTENSIONS.sub(_replace_extension, text)

    def replace_identifier(match: re.Match) -> str:
        tokens = _split_identifier(match.group(0))
        return ''.join(_token_to_kana(t) for t in tokens)

    return _IDENTIFIER.sub(replace_identifier, text)
...
if __name__ == "__main__":
    ...
    async def voice(
        ...
        logger.info(f"[normalize_english] 変換後テキスト: {text}")
